...
with open('testPredictions.csv', mode='w') as employee_file:
    ...
    for tile in range(0,4):
        ...
        yTrain = np.load(f'data/master/trainCropData{tile}.npy')
        xTrain = np.load(f'data/master/trainPixelData{tile}.npy')
        xTest = np.load(f'data/master/testPixelData{tile}.npy')
        testFieldData = np.load(f'data/master/testFieldData{tile}.npy')
